...
import pandas as pd
import posixpath
from pst_ag_project.settings import file_name
...
class RwjstspiderSpider(scrapy.Spider):
    ...
    def closed(self, reason):
        """To export scraped data to excel format

        Returns:
            excel file: An excel file exported from dataframe
        """
        df = pd.DataFrame(scraped_post_list)
        return df.to_excel(f"{self.name}_{file_name}.xlsx", index=False)
